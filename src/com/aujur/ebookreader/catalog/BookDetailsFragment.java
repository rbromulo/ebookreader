...
import com.aujur.ebookreader.activity.ReadingActivity;
...
public class BookDetailsFragment extends RoboSherlockFragment implements
	...
	@Inject
	private Provider<DownloadLocalFileTask> downloadLocalFileTaskProvider;
	...
	private void doSetFeed(Feed feed) {
		...
		if (entry.getEpubLink() != null) {
			...
			final String base = feed.getURL();

			try {

				if (!base.startsWith("vademecum")) {
					final URL url = new URL(new URL(base), entry.getEpubLink()
							.getHref());
					...
					addToLibraryButton
							.setOnClickListener(new View.OnClickListener() {
								@Override
								public void onClick(View view) {
									startLocalDownload(false, entry
											.getEpubLink().getHref());
								}
							});
		 ...
		 * 
		 * for ( final Link altLink: entry.getAlternateLinks() ) { TextView
		 * linkTextView = new TextView(getActivity());
		 * linkTextView.setTextAppearance( getActivity(),
		 * android.R.style.TextAppearance_Medium ); linkTextView.setText(
		 * altLink.getTitle() );
		 * linkTextView.setBackgroundResource(android.R.drawable
		 ...
		 * 
		 * @Override public void onClick(View view) { ((CatalogParent)
		 * getActivity()).loadFeedFromUrl(altLink.getHref()); } } );
		 * 
	...
	public void startLocalDownload(final boolean openOnCompletion,
			final String url) {
		...
		final DownloadLocalFileTask task = this.downloadLocalFileTaskProvider
				.get();
	...
	private abstract class AbstractDownloadCallback implements
			...
			DownloadLocalFileTask.DownloadFileCallback {
	...
	private class ProgressDialogCallback extends AbstractDownloadCallback
		...
		private DownloadLocalFileTask taskLocal;
		...
		private ProgressDialogCallback(Context context,
				DownloadLocalFileTask taskLocal, boolean openOnCompletion) {
